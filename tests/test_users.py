...
def test_creating_new_qualified_users_without_edit_qualifications(
    client, app, test_user, auth
):
    with app.app_context():
        user = User.query.get(1)
    ...
    auth.login("test")
    test_user.update(
        {"username": "a_new_user", "password": "123456", "edit_qualifications": False,}
    )
    response = client.post("/api/v1/users", json=test_user)
    assert response.status_code == 403
    assert response.is_json
...
def test_updating_user_permissions_subset(client, app, test_user, auth):
    ...
    auth.login("test")
    test_user.update(
        {"password": "123456", "edit_qualifications": False, "update_users": False,}
    )
    response = client.put("/api/v1/users/1", json=test_user)
    assert response.status_code == 200
    assert response.is_json
...
def test_updating_unqualified_users_without_edit_qualifications(
    client, app, test_user, auth
):
    with app.app_context():
        user = User.query.get(1)
...
def test_updating_qualified_users_without_edit_qualifications(
    client, app, test_user, auth
):
    with app.app_context():
        user = User.query.get(1)
...
def test_updating_qualified_users_with_edit_qualifications(
    client, app, test_user, auth
):
    auth.login("test")
    test_user.update(
